...
internals.findDynamoTypeMetadata = data => {
  ...
  if (meta) {
    ...
    if (data.type === 'alternatives' && data.alternatives.length === 2) {
      const notAny = data.alternatives.find(entry => entry.type !== 'any');
      return notAny ? internals.wireType(notAny.type) : null;
    } else {
