...
internals.createItem = (table, item, options, callback) => {
  ...
  internals.callBeforeHooks(self, 'create', start, (err, data) => {
    ...
    const attrs = utils.omitInvalid(data);
...
Table.prototype.update = function (item, options, callback) {
  ...
  // joi sucks. add a way to bypass it
  const schemaValidation = options[Symbol.for('dynogels.novalidate')] ? {} : internals.validateItemFragment(item, self.schema);
